...
card_generator.py
------------------
Builds shareable PNG images (the "bloom wrapped" recap and the streak card)
...
def make_wrapped_card(total_logs, total_pts, top_category, month_name):
    ...
    draw.text((WIDTH / 2, 220), "🌱 bloom", font=title_font, fill=COLORS["bark"], anchor="mm")
    ...
    draw.text((WIDTH / 2, HEIGHT - 70), "little by little, baby steps",
...
def make_streak_card(streak):
    ...
    draw.text((WIDTH / 2, 200), "🌱 bloom", font=_font(60, bold=True),
               fill=COLORS["bark"], anchor="mm")
